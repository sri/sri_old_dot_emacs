...
;; Author: <sri -at- asu.edu>
;; Last-modified: 2004-03-05T21:22:39
...
;; o modes for files .el files -> emacs lisp mode
...
(defun lynx-forward-follow (arg)
  (interactive "p")
  (let ((url (lynx-get-closest-url arg t)))
    (when url
      (lynx-fetch url))))
...
(add-hook 'kill-emacs-hook
  (lambda ()
    (while lynx-tmp-filenames
      (ignore-errors (delete-file (pop lynx-tmp-filenames))))))
...

;;; Recording actions for URLs.  If an URL loads up,
;;; and it has an associated action, then the action is executed.
;;; Convinent for moving to certain places (positions) in the
;;; buffer, highlighting headlines matching a regexp, or even
;;; inserting particular images....
  ...
  (let ((action
         (catch 'done
           (dolist (x lynx-command-actions)
             (when (string= (car x) url) ;string-match
               (throw 'done (cdr x)))))))
    (cond ((not action))
          ((or (stringp action) (vectorp action))
...
;;; Static:
(defmacro define-lynx-action (page &rest body)
  `(progn
     (push (cons ,page #'(lambda () ,@body)) lynx-command-actions)))
...
(defun lynx-record-action ()
  ...
  (if (not defining-kbd-macro)
      (let ((url (read-string "Record action for URL: ")))
        ...
        (lynx-fetch url)
        (start-kbd-macro nil)
        (message "Recording events..."))
    ;; Finish recording the macro...
    (end-kbd-macro)
    (push (cons lynx-record-action-url (copy-seq last-kbd-macro))
...
(defun lynx-google (string)
  (interactive "sSearch: ")
  (let* ((characters '((?\  . "+") (?\" . "%22")))
         (url (mapconcat (lambda (c)
                           (let ((new (cdr (assoc c characters))))
                             (if (stringp new)
                                 new
                               (string (or new c)))))
                         string "")))
    (lynx-fetch (format "http://www.google.com/search?q=%s&start=0"
                        url))))

(defun lynx-groups.google (string)
  )
